...
export class MultiAutoCompleteDemoArrayComponent implements OnInit {
  ...
  onSearchMultiple2(term: any) {
    return of(
      this.languages
        ...
        .filter(
          (lang) =>
            this.multiItems2.map((item) => item.id).indexOf(lang.id) === -1 && lang.label.toLowerCase().indexOf(term.toLowerCase()) !== -1
        )
    );
  }
  ...
  copy() {
    ...
    this.messages = [{ severity: 'success', summary: 'Copy success', detail: 'The data has been successfully copied to the clipboard.' }];
  }
}
