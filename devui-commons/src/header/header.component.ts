...
export class HeaderComponent implements OnInit {
  ...
  setSlideBarStyle(isClick?: Boolean): void {
    ...
    if (ele) {
      searchEle.setAttribute(
        'style',
        `display: ${this.showSlideMenu ? 'block' : 'none'} !important; max-width: ${this.showSlideMenu ? '230px' : '0'}`
      );
      footerEle.setAttribute(
        'style',
        `display: ${this.showSlideMenu ? 'flex' : 'none'} !important; max-width: ${this.showSlideMenu ? '249px' : '0'}`
      );
      if (isClick) {
        ele.setAttribute(
          'style',
          `max-width: ${this.showSlideMenu ? '250px' : '0'}; padding: ${
            this.showSlideMenu ? '0 8px 38px 8px' : '0'
          }; border-right: 1px solid var(--devui-dividing-line, #dfe1e6)`
        );
      } else {
